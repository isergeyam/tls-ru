...
streebog_a_values_ = [bytearray(b'\x8e \xfa\xa7+\xa0\xb4p'), bytearray(b'G\x10}\xdd\x9bPZ8'),
                      bytearray(b'\xad\x08\xb0\xe0\xc3(-\x1c'), bytearray(b'\xd8\x04Xp\xef\x14\x98\x0e'),
                      bytearray(b'l\x02,8\xf9\nL\x07'), bytearray(b'6\x01\x16\x1c\xf2\x05&\x8d'),
                      bytearray(b'\x1b\x8e\x0b\x0ey\x8c\x13\xc8'), bytearray(b'\x83G\x8b\x07\xb2F\x87d'),
                      bytearray(b'\xa0\x11\xd3\x80\x81\x8e\x8f@'), bytearray(b'P\x86\xe7@\xceG\xc9 '),
                      bytearray(b'(C\xfd g\xad\xea\x10'), bytearray(b'\x14\xaf\xf0\x10\xbd\xd8u\x08'),
                      bytearray(b'\n\xd9x\x08\xd0l\xb4\x04'), bytearray(b'\x05\xe2<\x04h6Z\x02'),
                      bytearray(b'\x8cq\x1e\x024\x1b-\x01'), bytearray(b'F\xb6\x0f\x01\x1a\x83\x98\x8e'),
                      bytearray(b'\x90\xda\xb5*8z\xe7o'), bytearray(b'Hm\xd4\x15\x1c=\xfd\xb9'),
                      bytearray(b'$\xb8j\x84\x0e\x90\xf0\xd2'), bytearray(b'\x12\\5B\x07Hxi'),
                      bytearray(b'\t.\x94!\x8d$<\xba'), bytearray(b'\x8a\x17J\x9e\xc8\x12\x1e]'),
                      bytearray(b'E\x85%Od\t\x0f\xa0'), bytearray(b'\xac\xcc\x9c\xa92\x8a\x89P'),
                      bytearray(b'\x9dM\xf0]_f\x14Q'), bytearray(b'\xc0\xa8x\xa0\xa13\n\xa6'),
                      bytearray(b'`T<P\xde\x97\x05S'), bytearray(b'0*\x1e(o\xc5\x8c\xa7'),
                      bytearray(b'\x18\x15\x0f\x14\xb9\xecF\xdd'), bytearray(b'\x0c\x84\x89\n\xd2v#\xe0'),
                      bytearray(b'\x06B\xca\x05i;\x9fp'), bytearray(
        b'\x03!e\x8c\xba\x93\xc18'), bytearray(b"\x86\']\xf0\x9c\xe8\xaa\xa8"), bytearray(b'C\x9d\xa0xNtUT'),
                      bytearray(b"\xaf\xc0P<\':\xa4*"), bytearray(b'\xd9`(\x1e\x9d\x1dR\x15'),
                      bytearray(b'\xe20\x14\x0f\xc0\x80)\x84'), bytearray(b'q\x18\n\x89`@\x9aB'),
                      bytearray(b'\xb6\x0c\x05\xca0 M!'), bytearray(b'[\x06\x8ce\x18\x10\xa8\x9e'),
                      bytearray(b'El4\x88z8\x05\xb9'), bytearray(b'\xac6\x1aD=\x1c\x8c\xd2'),
                      bytearray(b'V\x1b\r"\x90\x0eFi'), bytearray(b'+\x83\x88\x11H\x07#\xba'),
                      bytearray(b'\x9b\xcfD\x86$\x8d\x9f]'), bytearray(b'\xc3\xe9"C\x12\xc8\xc1\xa0'),
                      bytearray(b'\xef\xfa\x11\xaf\td\xeeP'), bytearray(b'\xf9}\x86\xd9\x8a2w('),
                      bytearray(b'\xe4\xfa T\xa8\x0b2\x9c'), bytearray(b'r}\x10*T\x8b\x19N'),
                      bytearray(b"9\xb0\x08\x15*\xcb\x82\'"), bytearray(b'\x92X\x04\x84\x15\xebA\x9d'),
                      bytearray(b'I,\x02B\x84\xfb\xae\xc0'), bytearray(b'\xaa\x16\x01!B\xf3W`'),
                      bytearray(b'U\x0b\x8e\x9e!\xf7\xa50'), bytearray(b'\xa4\x8bGO\x9e\xf5\xdc\x18'),
                      bytearray(b'p\xa6\xa5n$@Y\x8e'), bytearray(b'8S\xdc7\x12 \xa2G'),
                      bytearray(b'\x1c\xa7n\x95\t\x10Q\xad'), bytearray(b'\x0e\xdd7\xc4\x8a\x08\xa6\xd8'),
                      bytearray(b'\x07\xe0\x95bE\x04Sl'), bytearray(b'\x8dp\xc41\xac\x02\xa76'),
                      bytearray(b'\xc88b\x96V\x01\xdd\x1b'), bytearray(b'd\x1c1K+\x8e\xe0\x83')]

streebog_C_values = [bytearray(
...
def streebog_LPS_precomputation_():
    ...
    streebog_LPS_values = [[0 for j in range(256)] for i in range(64)]
...
class StreebogHasher:
    ...
    def __invert__(self):
        ...
        res = self.Step3_const(self.buffer, self.buffer_size)
        self.buffer[63 - self.buffer_size] = 0
        return res
    ...
    def Step3_const(self, m: bytearray, offset):
        M = toint(m)
        Sigma = self.Sigma
        h = self.h
        N = self.N
